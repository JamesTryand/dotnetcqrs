...
/// <summary>Thrown by every write method on a <see cref="SqliteEventStore"/> opened via
/// <see cref="SqliteEventStore.OpenReadOnlyAsync"/> — a same-host secondary polls the
/// master's <c>events.db</c> directly and must never append to it (or checkpoint/dead-letter
/// into it) itself; see <see cref="SqliteEventStore.OpenReadOnlyAsync"/>'s own doc comment
