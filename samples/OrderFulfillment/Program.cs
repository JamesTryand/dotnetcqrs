...
await using var readModelDb = await ReadModelDb.OpenAsync(readModelPath);
...
await PrintTableAsync(readModelDb, "orders", "order_id", "title", "confirmed");
await PrintTableAsync(readModelDb, "tasks", "task_id", "title", "completed");

Console.WriteLine();
Console.WriteLine("-- the write-guard, for real: a direct write to a projection-owned table --");
try
{
    await using var direct = readModelDb.CreateCommand();
...
static async Task PrintTableAsync(SqliteConnection db, string table, params string[] columns)
{
